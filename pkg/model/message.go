...
type SendMessageRequest struct {
	...
	// AlsoSendToChannel requests that a thread reply also appear in the parent
	// room's channel timeline (the "Also send to channel" option). Only
	// meaningful when ThreadParentMessageID is set — message-gatekeeper
	// normalizes it to false on non-thread sends. Maps onto Message.TShow.
