...
func TestReactions_MarshalJSON_FlatArray_Sorted(t *testing.T) {
	now := time.Now().UTC().Truncate(time.Millisecond)
	r := Reactions{
...
func TestReactions_MarshalJSON_EmptyMap(t *testing.T) {
	r := Reactions{}
	data, err := json.Marshal(r)
	require.NoError(t, err)
...
func TestReactions_MarshalJSON_NilMap_OmittedViaOmitempty(t *testing.T) {
	msg := Message{
		RoomID:    "r1",
		CreatedAt: time.Date(2026, 1, 1, 0, 0, 0, 0, time.UTC),
		MessageID: "m1",
...
func TestReactions_UnmarshalJSON_HappyPath(t *testing.T) {
	now := time.Now().UTC().Truncate(time.Millisecond)
	want := Reactions{
		ReactionKey{Emoji: "👍", UserAccount: "alice"}: ReactorInfo{
			UserID: "u1", EngName: "Alice", ChnName: "爱丽丝", Account: "alice", ReactedAt: now,
...
func TestReactions_UnmarshalJSON_NullYieldsNil(t *testing.T) {
	r := Reactions{ReactionKey{Emoji: "x", UserAccount: "a"}: ReactorInfo{UserID: "u"}}
	require.NoError(t, r.UnmarshalJSON([]byte("null")))
	assert.Nil(t, r)
}

func TestReactions_UnmarshalJSON_EmptyArrayYieldsEmptyMap(t *testing.T) {
	var r Reactions
...
func TestReactions_UnmarshalJSON_DuplicateKey_ReturnsError(t *testing.T) {
	raw := []byte(`[
...
func TestReactions_UnmarshalJSON_MalformedJSON(t *testing.T) {
	var r Reactions
	err := r.UnmarshalJSON([]byte(`{not valid json`))
	require.Error(t, err)
	assert.Contains(t, err.Error(), "reactions: unmarshal:")
}
