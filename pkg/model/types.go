...
func verifyUDTTags(samplePtr any) {
	...
	if rv.Kind() != reflect.Ptr {
		panic(fmt.Sprintf("model: verifyUDTTags requires a pointer, got %T", samplePtr))
	}
	t := rv.Elem()
	if t.Kind() != reflect.Struct {
		panic(fmt.Sprintf("model: verifyUDTTags requires a pointer to struct, got pointer to %s", t.Kind()))
	}
	for i := range t.NumField() {
		f := t.Field(i)
		if f.Tag.Get("cql") == "" {
			panic(fmt.Sprintf("model: struct %s field %s is missing a `cql` tag", t.Name(), f.Name))
		}
	}
}
