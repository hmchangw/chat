// Package pipelines holds shared MongoDB query predicates used by more than one
// service. Centralizing the "which users does this add/create-member request
// resolve to" filter keeps room-service (capacity check) and room-worker
// (candidate resolution) in lock-step on org expansion and bot exclusion.
package pipelines
...
// accounts. It is the wire-side equivalent of model.IsBotAccount, applied as a
// residual filter so org-expanded candidates — whose accounts the caller does
// not know up front — are excluded server-side.
