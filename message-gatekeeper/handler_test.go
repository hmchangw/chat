...
func TestHandler_ProcessMessage(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// The gatekeeper no longer carries the thread parent's createdAt on the
			// canonical event: each downstream consumer re-resolves it from an
			// authoritative store (message-worker from Cassandra, search-sync-worker
			// from Elasticsearch, notification-worker from thread_rooms). A client
			// value, if still sent, is an unknown JSON field that is simply ignored.
		...
		{
			// A thread reply with no parent timestamp is now valid — the gatekeeper
			// dropped the required-field check; downstream re-resolution owns the
			// authoritative value.
...
func TestHandler_ProcessMessage_WithQuote(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			// the quote is fetched. The thread parent's createdAt is no longer carried
			// on the canonical event — downstream re-resolves it.
