...
// startReplicaSet stands up a single-node Mongo replica set (required for
// change streams) and returns a connected client plus its URI. Inline per the
// CLAUDE.md exception: testutil only provides standalone Mongo, which cannot
// Watch.
...
// TestConnector_RealPublishEndToEnd runs the full connector (start → real NATS
// publish) and reads the envelope back off MIGRATION_OPLOG — covering main.go's
// wiring and the real oteljetstream publish path.
...
// TestOplogConnector_ChangeStreamEndToEnd drives a real change stream through
// the real watcher and asserts the published envelopes — exercising
// source_mongo.go + handler.go + envelope.go against live CDC.
