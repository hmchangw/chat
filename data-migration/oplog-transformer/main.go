...
// — then Termed with a distinct metric instead of JetStream's silent drop (see isFinalDelivery).
func processOne(ctx context.Context, h *handler, m jetstream.Msg, mtr *metrics, maxDeliver, deleteMaxDeliver int) {
	...
	case errors.Is(err, errSkipped):
		...
		dispose("ack", m.Ack)
	case isFinalDelivery(m, deliverCap):
...
// isFinalDelivery reports whether this is the last delivery (NumDelivered ≥ maxDeliver), so a further
// Nak would be a silent drop. maxDeliver ≤ 0 means unlimited; a Metadata error prefers Nak over a premature Term.
func isFinalDelivery(m jetstream.Msg, maxDeliver int) bool {
	if maxDeliver <= 0 {
		return false
	}
	meta, err := m.Metadata()
