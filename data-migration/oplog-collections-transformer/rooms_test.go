...
func TestHandleRoom_InsertChannel(t *testing.T) {
	...
	doc := `{"_id":"r1","t":"c","fname":"General","name":"general","ro":false,"uids":["u1","u2"],"usernames":["alice","bob"]}`
...
func TestHandleRoom_UpdateRestrictedChange(t *testing.T) {
	pub := &fakePublisher{}
	full := `{"_id":"r1","t":"c","fname":"Room","ro":true,"uids":["u1"]}`
	h := newTestHandler(pub, &fakeTarget{}, &fakeLookup{doc: json.RawMessage(full)})

	err := h.handleRoom(context.Background(), roomEv("update", "", `{"updatedFields":{"ro":true}}`))
	require.NoError(t, err)

	// ro change → [room_restricted, room_sync]
	require.Len(t, pub.events, 2)
	evt := pub.events[0]
...
func TestHandleRoom_UpdateRestrictedChangeEmitsBothEvents(t *testing.T) {
	pub := &fakePublisher{}
	full := `{"_id":"r1","t":"c","fname":"Room","ro":true,"uids":["u1"],"_updatedAt":{"$date":"2024-05-01T12:00:00.000Z"}}`
	h := newTestHandler(pub, &fakeTarget{}, &fakeLookup{doc: json.RawMessage(full)})
