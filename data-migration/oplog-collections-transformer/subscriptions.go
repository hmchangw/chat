...
func (h *handler) handleSubscription(ctx context.Context, ev oplogEvent) error {
	if ev.Op == "delete" {
		...
		return h.pub.Publish(ctx, h.subscriptionDeletedEvent(id))
	}
...
func (h *handler) handleSubscriptionUpdate(ctx context.Context, ev oplogEvent, ss *sourceSubscription) error {
	...
	if subChanged(desc, "roles") && len(ss.Roles) > 0 {
...
func (h *handler) memberAddedEvent(ss *sourceSubscription, siteID string) model.InboxEvent {
	...
	payload := mustMarshal(model.MemberAddEvent{
		...
		RoomName:  ss.FName,
		SiteID:    siteID,
		SubID:     ss.ID, // destination adopts the source _id so a later hard-delete maps back.
...
func (h *handler) subscriptionDeletedEvent(subID string) model.InboxEvent {
	...
	return h.inboxEvent(model.InboxSubscriptionDeleted, h.siteID, payload)
}
...
// else (RC "moderator"/"leader"/"user", which the new model lacks) → RoleMember.
