...
func run() error {
	...
	printResponse(request(nc, msgHistory(username, roomID, siteID), historyReq, timeout))
	...
	printResponse(request(nc, msgHistory(username, roomID, siteID), historyBefore, timeout))
	...
	printResponse(request(nc, msgNext(username, roomID, siteID), nextReq, timeout))

	// --- 4. GetMessageByID - file message (with createdAt hint for O(1) lookup) ---
	header("4. GetMessageByID - msg-002 (file attachment, fast path)")
	getFileReq, _ := json.Marshal(map[string]any{
		"messageId": "msg-002",
		"createdAt": msgs[2].time.UnixMilli(),
	})
	printResponse(request(nc, msgGet(username, roomID, siteID), getFileReq, timeout))

	// --- 5. GetMessageByID - card message (with createdAt hint) ---
	header("5. GetMessageByID - msg-003 (CI report card, fast path)")
	getCardReq, _ := json.Marshal(map[string]any{
		"messageId": "msg-003",
		"createdAt": msgs[3].time.UnixMilli(),
	})
	printResponse(request(nc, msgGet(username, roomID, siteID), getCardReq, timeout))

	// --- 6. GetMessageByID - card action message (without createdAt — slow path fallback) ---
	header("6. GetMessageByID - msg-007 (card action + reactions, slow path)")
	getActionReq, _ := json.Marshal(map[string]any{
		"messageId": "msg-007",
	})
	printResponse(request(nc, msgGet(username, roomID, siteID), getActionReq, timeout))

	// --- 7. GetMessageByID - edited + federated message (with createdAt hint) ---
	header("7. GetMessageByID - msg-010 (sys_msg + federated + mentions, fast path)")
	getSysReq, _ := json.Marshal(map[string]any{
		"messageId": "msg-010",
		"createdAt": msgs[10].time.UnixMilli(),
	})
	printResponse(request(nc, msgGet(username, roomID, siteID), getSysReq, timeout))

	// --- 8. LoadSurroundingMessages (with createdAt hint for O(1) central message lookup) ---
	header("8. LoadSurroundingMessages - context around msg-005 (fast path)")
	info("3 before + msg-005 + 3 after = up to 7 messages")
	surroundReq, _ := json.Marshal(map[string]any{
		"messageId": "msg-005",
		"createdAt": msgs[5].time.UnixMilli(),
		"limit":     7,
	})
	printResponse(request(nc, msgSurrounding(username, roomID, siteID), surroundReq, timeout))
	...
	printResponse(request(nc, msgGet(username, roomID, siteID), notFoundReq, timeout))

	header("10. Error: not subscribed (unknown user)")
	notSubReq, _ := json.Marshal(map[string]any{
		"limit": 5,
	})
	printResponse(request(nc, msgHistory("unknown-user", roomID, siteID), notSubReq, timeout))
...
// Subject builders for the demo client (mirrors the history-service natsrouter patterns).
