...
- Stream creation is gated by `BOOTSTRAP_STREAMS` (see below); when enabled, use `js.CreateOrUpdateStream` (it's schema-idempotent; preserves federation config from existing stream) via the service's `bootstrapStreams` helper, never inline
...
- **Stream bootstrap ownership.** When a service does bootstrap a stream in dev, the helper authors only `Name + Subjects` from `pkg/stream.<Stream>(siteID)` + a union over any existing `Sources` from the live stream (preserve, never author). Federation config (`Sources` + `SubjectTransforms` for cross-site sourcing) is **owned by ops/IaC and MUST NOT be authored** by any service's `bootstrap.go` — services never invent or hard-code federation values. When the helper finds the stream already exists, it reads the existing `Sources` (with their nested `SubjectTransforms`) from the live `StreamInfo` and merges them verbatim into the schema-only update, so a worker restart doesn't silently clear what ops/IaC (or the e2e harness) layered on. The narrow contract is "author nothing; preserve what's already there." See `inbox-worker/bootstrap.go` for the reference implementation. INBOX has a single owning service (`inbox-worker`); other services that consume from INBOX (e.g., `search-sync-worker`) skip it in their bootstrap loop and rely on `inbox-worker` to create the stream.
