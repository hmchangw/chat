...
| `USE_INFRA=true make -C tools/integration-suite-multisite local` | Run the integration suite against a programmatically-booted 26-container two-site stack (testcontainers-go). Sparse `sites:` maps mean single-site-shape scenarios run cleanly without paying cross-site cost. See Section 6 + `tools/integration-suite-multisite/RUNBOOK.md`. (The legacy single-site tool lives at `tools/archived/integration-suite/` — feature reference only, not maintained; see its `STALE.md`.) |
...
- **Scope rule — the suite team does NOT modify chat-app code.** Work here lives under `tools/integration-suite-multisite/` and the suite's own docs (`docs/integration-suite-*.md`). When a finding requires a chat-app change (e.g., the F-009 in-process-cache reset hook in `integration-suite-multisite-findings.md`), file it in `docs/integration-suite-multisite-findings.md` for the chat-app team and ship the strongest tool-side mitigation that doesn't require chat-app changes. Editing chat-app services, `pkg/`, or other tools' source is out of scope without explicit instruction.
- **Multi-site is the active suite for ALL integration testing**, including single-site-shape scenarios. The `sites:` map is sparse: a scenario that declares only `sites: {site-a: ...}` (no `site-b` block) runs cleanly without paying any cross-site cost. The `infra-sanity-rooms-pipeline-site-a` scenario is the canonical single-site example under the multi-site runner.
...
- **Findings** — durable chat-app-team-facing reports from the suite live in `docs/integration-suite-multisite-findings.md`. The forward-looking design note `docs/integration-suite-plan-ahead.md` is the north star for the envelope + DAG + chaos engine direction; not yet specced.
- **Legacy single-site suite** at `tools/archived/integration-suite/` is preserved as a feature reference (cases grammar, mishaps, single-purpose reader names) but **not maintained** — see its `STALE.md`. Do not add new scenarios there.
