...
func TestBuildRoomEvent(t *testing.T) {
	room := &model.Room{
		ID:        "room-42",
		Name:      "engineering",
	...
	msgTime := time.Date(2026, 4, 1, 9, 0, 0, 0, time.UTC)
	msg := &model.Message{
		ID:        "msg-99",
		RoomID:    "room-42",
		UserID:    "u9",
	...
	assert.Equal(t, model.RoomEventNewMessage, evt.Type)
	assert.Equal(t, "room-42", evt.RoomID)
	assert.Equal(t, msgTime, evt.Timestamp)
	assert.Equal(t, "engineering", evt.RoomName)
	assert.Equal(t, model.RoomTypeGroup, evt.RoomType)
	assert.Equal(t, "site-b", evt.SiteID)
	assert.Equal(t, 12, evt.UserCount)
	assert.Equal(t, msgTime, evt.LastMsgAt)
	assert.Equal(t, "msg-99", evt.LastMsgID)
	require.NotNil(t, evt.Message)
	assert.Equal(t, clientMsg, evt.Message)
	assert.False(t, evt.MentionAll)
	assert.False(t, evt.HasMention)
	assert.Empty(t, evt.Mentions)
}

func TestBuildRoomEvent_DMRoom(t *testing.T) {
	room := &model.Room{
		ID:        "dm-99",
		Name:      "",
	...
	clientMsg := &model.ClientMessage{
		Message: model.Message{
			ID: "dm-msg-1", RoomID: "dm-99", UserID: "u1", Username: "alice",
			Content: "hey", CreatedAt: msgTime,
		},
		Sender: &model.Participant{UserID: "u1", Username: "alice", ChineseName: "愛麗絲", EngName: "Alice Wang"},
	}

	evt := buildRoomEvent(room, clientMsg)

	assert.Equal(t, "dm-99", evt.RoomID)
	assert.Equal(t, model.RoomTypeDM, evt.RoomType)
	assert.Equal(t, "site-c", evt.SiteID)
	assert.Equal(t, msgTime, evt.Timestamp)
	assert.Equal(t, msgTime, evt.LastMsgAt)
	assert.Equal(t, "dm-msg-1", evt.LastMsgID)
	assert.Empty(t, evt.RoomName)
	assert.Equal(t, clientMsg, evt.Message)
}

func TestBuildMentionParticipants_EmptySlice(t *testing.T) {
	employees := map[string]model.Employee{
		"alice": {AccountName: "alice", Name: "愛麗絲", EngName: "Alice Wang"},
	}
	// Empty slice (not nil) should also return nil
	result := buildMentionParticipants([]string{}, employees)
	assert.Nil(t, result)
}

func TestBuildClientMessage_MessageFieldsPreserved(t *testing.T) {
	msgTime := time.Date(2026, 4, 1, 8, 0, 0, 0, time.UTC)
	msg := &model.Message{
		ID:        "m-preserve",
		RoomID:    "r-preserve",
		UserID:    "u-preserve",
	...
	employees := map[string]model.Employee{
		"diana": {AccountName: "diana", Name: "黛安娜", EngName: "Diana Lu"},
	}

	cm := buildClientMessage(msg, employees)

	// All original Message fields must be preserved unchanged
	assert.Equal(t, "m-preserve", cm.ID)
	assert.Equal(t, "r-preserve", cm.RoomID)
	assert.Equal(t, "u-preserve", cm.UserID)
	assert.Equal(t, "diana", cm.Username)
	assert.Equal(t, "check all fields", cm.Content)
	assert.Equal(t, msgTime, cm.CreatedAt)
	// Sender is enriched
	require.NotNil(t, cm.Sender)
	assert.Equal(t, "黛安娜", cm.Sender.ChineseName)
	assert.Equal(t, "Diana Lu", cm.Sender.EngName)
}

func TestHandler_HandleMessage_DMRoom_SenderEnrichment(t *testing.T) {
	msgTime := time.Date(2026, 3, 26, 14, 0, 0, 0, time.UTC)
	...
	for _, rec := range pub.records {
		...
		assert.Equal(t, "alice-id", roomEvt.Message.Sender.UserID)
		assert.Equal(t, "alice", roomEvt.Message.Sender.Username)
		assert.Equal(t, "愛麗絲", roomEvt.Message.Sender.ChineseName)
		assert.Equal(t, "Alice Wang", roomEvt.Message.Sender.EngName)
	}
}
