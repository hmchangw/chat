...
func TestHandleThreadCreated_DMRoom_WithMention_SetsMentions(t *testing.T) {
	ctrl := gomock.NewController(t)
	store := NewMockStore(ctrl)
	us := NewMockUserStore(ctrl)
	pub := &mockPublisher{}
...
func TestHandleCreated_RoomRenamed_PublishesRoomRenamedEvent(t *testing.T) {
	ctrl := gomock.NewController(t)
	store := NewMockStore(ctrl)
	us := NewMockUserStore(ctrl)
	pub := &mockPublisher{}
	...
	siteID := "site-a"
	renamedAt := time.Date(2026, 6, 17, 10, 0, 0, 0, time.UTC)

	sysData, err := json.Marshal(model.RoomRenamedSysData{NewName: "new-channel-name", ByAccount: "alice"})
	require.NoError(t, err)
	...
	evt := model.MessageEvent{
		...
		Message: model.Message{
			...
			Content:     `"alice" renamed the channel to "new-channel-name"`,
			SysMsgData:  sysData,
			CreatedAt:   renamedAt,
		},
	}
	...
	// handleCreated: user lookup, update last message, get room meta for the sys message fan-out.
	store.EXPECT().UpdateRoomLastMessage(gomock.Any(), roomID, "msg-renamed-1", renamedAt, false).Return(nil)
	store.EXPECT().GetRoomMeta(gomock.Any(), roomID).Return(metaOf(&model.Room{
		ID: roomID, Type: model.RoomTypeChannel, SiteID: siteID, Name: "old-channel-name",
	}), nil)
	...
	// Expect exactly 2 publishes: RoomRenamedRoomEvent first, then the sys-message new_message event.
	require.Len(t, pub.records, 2, "rename must publish RoomRenamedRoomEvent + sys-message new_message event")

	// records[0]: flat RoomRenamedRoomEvent on chat.room.{roomID}.event.
	assert.Equal(t, subject.RoomEvent(roomID), pub.records[0].subject)
	var renamed model.RoomRenamedRoomEvent
	require.NoError(t, json.Unmarshal(pub.records[0].data, &renamed))
	assert.Equal(t, model.RoomEventRoomRenamed, renamed.Type)
	assert.Equal(t, roomID, renamed.RoomID)
	assert.Equal(t, siteID, renamed.SiteID)
	assert.Equal(t, "new-channel-name", renamed.NewName)
	assert.Equal(t, "alice", renamed.ByAccount)
	assert.True(t, renamed.RenamedAt.Equal(renamedAt), "RenamedAt must equal msg.CreatedAt")
	assert.Positive(t, renamed.Timestamp, "Timestamp must be set to publish time (ms)")

	// Flat shape: verify the raw JSON contains no zero-valued RoomEvent base fields.
	var raw map[string]json.RawMessage
	require.NoError(t, json.Unmarshal(pub.records[0].data, &raw))
	for _, zeroField := range []string{"userCount", "lastMsgAt", "lastMsgId", "message", "mentions", "mentionAll"} {
		assert.NotContains(t, raw, zeroField, "flat event must not include zero-valued base field %q", zeroField)
	}

	// records[1]: normal sys-message new_message event for scrollback — must still be emitted.
	assert.Equal(t, subject.RoomEvent(roomID), pub.records[1].subject)
	var sysEvt model.RoomEvent
	require.NoError(t, json.Unmarshal(pub.records[1].data, &sysEvt))
	assert.Equal(t, model.RoomEventNewMessage, sysEvt.Type)
}

func TestHandleCreated_RoomRenamed_MissingSysMsgData_ReturnsError(t *testing.T) {
	ctrl := gomock.NewController(t)
	store := NewMockStore(ctrl)
	us := NewMockUserStore(ctrl)
	pub := &mockPublisher{}
	...
	err = h.HandleMessage(context.Background(), data)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "room_renamed sys data")
	assert.Empty(t, pub.records, "no publish on malformed sys data")
}
