...
## F-019 — publish-time authorization not enforced in the suite NATS (sender impersonation succeeds)

**Layer:** suite NATS configuration **or** production NATS configuration —
not chat-app code. Distinguishing the two requires runtime verification
(see §Recommended next steps); chat-app sources are clean.

**Status:** observed — needs runtime verification before triage. The
empirical symptom is reproducible in the suite; whether it also reproduces
in production is unknown until ops/security checks the production NATS
account/operator config.

**Reachability:** any user authenticated with their own correctly-scoped
JWT can publish to any other user's `chat.user.{account}.>` subject; the
gatekeeper attributes the message to the impersonated user (since
gatekeeper trusts the subject's `{account}` token for sender identity).
Two `msg.send` publishes from one alice-authenticated connection — one to
alice's own subject (control), one to bob's subject (impersonate) — both
produce canonical events.

The defense for cross-user publish-time authorization lives **entirely** in
NATS enforcing the user JWT's Pub.Allow. The gatekeeper deliberately
trusts the subject's `{account}` token (handler.go:81) because that token
can only be in the subject if NATS allowed the connection to publish
there — i.e., the user's JWT Pub.Allow covers it. The payload of
`SendMessageRequest` has no sender field; subject is the sole source of
identity for canonical attribution.

Each link in the trust chain verified clean by static analysis:

| Link | Verified |
|---|---|
| Spoofable sender in payload | No — `SendMessageRequest` = `{ID, Content, RequestID, thread/quote}`; no sender field |
| auth-service mints broad JWT | No — `auth-service/handler.go:234` sets `Pub.Allow = chat.user.{account}.>` (+ `_INBOX.>`, presence-query) |
| `${alice.credential}` resolves to a broad service cred | No — `runner_scenario.go:328-336` returns the user's own JWT+nkey-seed; service creds need `service.` prefix |
| Publish uses a shared/admin connection | No — `verbs/jetstream_publish.go:43-58` opens a per-call connection via `nats.UserJWTAndSeed(alice.JWT, alice.seed)` |
| NATS in permissive mode | No — `docker-local/nats.conf` (mounted into suite testcontainer at `internal/infra/deps.go:40`) uses operator + MEMORY resolver + account JWTs, no `no_auth_user` |
| JWT silently dropped as untrusted | No — the control publish (alice→alice's own namespace) succeeds, so the JWT IS trusted by NATS |

Net: alice connects with a trusted, correctly-scoped JWT yet publishes
outside her namespace. In standard NATS decentralized auth that is
impossible — user JWT claims are enforced directly by `nats-server` at
publish time. The empirical fact contradicts the static analysis, so
either the JWT being minted is not what we think (silently blank or
wildcard `Pub.Allow`), or `nats-server` is not enforcing user-JWT claims
under this specific config, or something in the publish path rewrites the
subject. Static analysis cannot distinguish the three.

**Two interpretations (needs triage):**

1. **Local-dev-config gap (testing-only).** The suite reuses
   `docker-local/nats.conf` — generated by `docker-local/setup.sh`. That
   setup may produce a more permissive trust chain than what ops/IaC runs
   in production. If production NATS enforces user Pub.Allow correctly,
   impersonation is blocked there and this is a suite-fidelity gap; the
   fix is in the suite's setup (regenerate the chatapp account JWT with a
   signing key delegation via `nsc edit account chatapp --sk <pubkey>`,
   or whatever else the runtime check pins).
2. **Real production vulnerability.** If the same trust chain is used
   in production, any authenticated user can post as any other user — a
   critical impersonation vulnerability. The auth-service design
   explicitly relies on pub-scoping (handler.go:243-247 inline: *"so
   presence can't be forged"*); the intended invariant is
   enforcement-on. The fix is in the NATS account/operator config, not
   the chat-app code (auth-service already issues the correctly-scoped
   JWT).

Only the production NATS config (owned by ops/IaC, outside the suite's
...
1. **Runtime verification (5 minutes, pins the cause).** Two commands
   against the running suite NATS:
   - Decode a minted user JWT. If `Pub.Allow` shows
     `["chat.user.alice.>", "_INBOX.>", ...]`, the JWT is correct and
     `nats-server` is at fault. If `Pub.Allow` is empty or `[">"]`, the
     mint is at fault.
   - Independently test enforcement with the bare `nats` CLI:
     `nats pub --creds <alice.creds> 'chat.user.bob.room.test.msg.send' '{"x":1}'`.
     If it succeeds, `nats-server` is definitively not enforcing user-JWT
     Pub.Allow. If it fails with a permissions violation but the suite's
     publish succeeds, the bug is in the suite's connection path.
2. **Ops/security action (gated on #1).** If runtime verification points
   at `nats-server` not enforcing, verify whether production NATS uses
   the same setup. If yes → real vulnerability; fix in NATS
   account/operator config. Auth-service issues the correctly-scoped
   JWT already.
3. **Tool-side action (gated on #1).** If runtime verification points at
   the JWT mint, file a chat-app bug against auth-service. If it points
   at a suite-config gap, fix `docker-local/setup.sh` (probably an `nsc
   edit account --sk` delegation step) and the canary turns green.

**The decision the team owns:** confirm whether the suite's NATS is
meant to enforce decentralized user pub permissions. If yes → run the
