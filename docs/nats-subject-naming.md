...
| `chat.msg.canonical.{siteID}.reacted` | history-service | broadcast-worker, notification-worker | Reaction toggled (add/remove). Payload carries a `ReactionDelta`; search-sync-worker skips these (no content change). |
