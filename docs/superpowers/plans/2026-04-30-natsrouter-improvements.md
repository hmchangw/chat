...
## Task 4: Per-route `WithConcurrency` override

**Why:** A noisy or expensive route can saturate the global semaphore and starve cheap routes. Give each `Register` call the option to opt into a route-private semaphore so that route's saturation no longer affects others. Default behavior (no option) still uses the router-wide semaphore — no breaking change for existing callers.

**Files:**
- Modify: `pkg/natsrouter/router.go` (extend `Registrar.addRoute` signature; add `routeConfig`, `RouteOption`, `WithConcurrency`)
- Modify: `pkg/natsrouter/register.go` (accept `...RouteOption` in `Register`, `RegisterNoBody`, `RegisterVoid`; pass through)
- Modify: `pkg/natsrouter/router_test.go` or integration test (verify per-route sem isolation)

- [ ] **Step 1: Write the failing isolation integration test**

Append to `pkg/natsrouter/integration_test.go`:

```go
// TestIntegration_PerRouteConcurrencyIsolation verifies that a route
// registered with WithConcurrency(1) saturates independently of the
// router-wide semaphore, so other routes remain available.
func TestIntegration_PerRouteConcurrencyIsolation(t *testing.T) {
	nc := setupNATS(t)
	r := natsrouter.New(nc, "integration-iso", natsrouter.WithMaxConcurrency(8))

	slow := make(chan struct{})
	natsrouter.Register(r, "slow.{id}",
		func(c *natsrouter.Context, req echoReq) (*echoResp, error) {
			<-slow
			return &echoResp{Seq: req.Seq}, nil
		},
		natsrouter.WithConcurrency(1),
	)

	natsrouter.Register(r, "fast.{id}",
		func(c *natsrouter.Context, req echoReq) (*echoResp, error) {
			return &echoResp{Seq: req.Seq}, nil
		})

	// Saturate the slow route.
	go func() {
		data, _ := json.Marshal(echoReq{Seq: 0})
		_, _ = nc.Request(context.Background(), "slow.0", data, 5*time.Second)
	}()

	// Confirm slow route now busy-replies (route-private cap=1).
	require.Eventually(t, func() bool {
		data, _ := json.Marshal(echoReq{Seq: 1})
		resp, err := nc.Request(context.Background(), "slow.1", data, 1*time.Second)
		if err != nil {
			return false
		}
		var re natsrouter.RouteError
		_ = json.Unmarshal(resp.Data, &re)
		return re.Code == natsrouter.CodeUnavailable
	}, 5*time.Second, 50*time.Millisecond)

	// Fast route must still serve normally.
	data, _ := json.Marshal(echoReq{Seq: 99})
	resp, err := nc.Request(context.Background(), "fast.7", data, 2*time.Second)
	require.NoError(t, err)
	var ok echoResp
	require.NoError(t, json.Unmarshal(resp.Data, &ok))
	assert.Equal(t, 99, ok.Seq)
...
Run: `go test -tags=integration ./pkg/natsrouter/ -run TestIntegration_PerRouteConcurrencyIsolation -v`
Expected: FAIL — `WithConcurrency` and `RouteOption` undefined.

- [ ] **Step 3: Add `routeConfig`, `RouteOption`, `WithConcurrency` and extend `Registrar`**

In `pkg/natsrouter/router.go`:

Replace the existing `Registrar` block:
...
```go
// Registrar is the interface for registering route handlers. Internal
// detail — only *Router implements it, but it's exported so generic
// helpers (Register, RegisterNoBody, RegisterVoid) can accept routers
// constructed by callers.
type Registrar interface {
	addRoute(pattern string, handlers []HandlerFunc, cfg routeConfig)
}

// routeConfig carries per-route options resolved at registration time.
type routeConfig struct {
	// sem is a route-private semaphore; nil means the route uses the
	// router's shared semaphore.
	sem chan struct{}
}
...
// WithConcurrency gives this route its own admission-control semaphore of
// the given capacity, isolating it from the router-wide pool. Use for
// expensive or unreliable routes that should not be able to starve cheaper
// routes. Non-positive values are ignored (route falls back to the
// router-wide semaphore).
func WithConcurrency(n int) RouteOption {
...
Replace the existing `addRoute` method:
...
```go
func (r *Router) addRoute(pattern string, handlers []HandlerFunc, cfg routeConfig) {
	rt := parsePattern(pattern)
	all := make([]HandlerFunc, 0, len(r.middleware)+len(handlers))
	all = append(all, r.middleware...)
	all = append(all, handlers...)
...
(Keep the rest of `addRoute` — the `subs` append + lock — unchanged.)

- [ ] **Step 4: Update `Register`, `RegisterNoBody`, `RegisterVoid` to accept route options**

In `pkg/natsrouter/register.go`, replace each function signature and the `addRoute` call inside.
...
func Register[Req, Resp any](
	...
	r.addRoute(pattern, []HandlerFunc{handler}, cfg)
}
```

`RegisterNoBody`: same pattern — append `opts ...RouteOption` to the signature, build `cfg`, pass to `addRoute`.

`RegisterVoid`: same pattern.
...
go test -tags=integration ./pkg/natsrouter/...
```
Expected: PASS — new isolation test passes; all existing tests still green; existing `Register(r, p, fn)` call sites still typecheck because `opts` is variadic.

- [ ] **Step 6: Verify all dependent services still build**

Run from repo root: `go build ./...`
Expected: PASS — every service that calls `natsrouter.Register(...)` continues to compile.

- [ ] **Step 7: Commit**

```bash
git add pkg/natsrouter/router.go pkg/natsrouter/register.go pkg/natsrouter/integration_test.go
git commit -m "feat(natsrouter): per-route WithConcurrency override"
```
