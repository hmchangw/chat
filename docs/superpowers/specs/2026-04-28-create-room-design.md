...
- Channels require a client-supplied `Name`; `Name == ""` for `roomType == channel` is rejected with the new sentinel `errChannelNameRequired`. The server never auto-generates a channel name. (DM/botDM rooms never carry a request `Name`; their `Room.Name` is `RoomID`.)
...
- **`Name`**: channels — request `Name` (required, non-empty), truncated to 100 runes via `truncateRunes`; DM/botDM — equal to `ID`. Display-name-per-subscriber for DM/botDM lives on `Subscription.SidebarName`, not `Room.Name`.
...
`truncateRunes` is the only name normalizer used at create-room time. Channels require a client-supplied `Name` (validated in room-service); the server never composes a channel name from the user/org/channel lists.
...
| 11 | Channel name truncation | client `Name` longer than 100 runes | publish; payload `Name` exactly 100 runes (via `truncateRunes`) |
...
- Channel `Name` longer than 100 runes → Room doc carries the truncated form (`truncateRunes(req.Name, 100)`).
