...
- Channel auto-name composition (`composeAutoName`): when `Name == ""`, joins the literal request `users + orgs + channelRefs.RoomID` values with `", "` and truncates to 100 runes.
...
- **`ID`**: channels use `idgen.GenerateID()` (17-char base62) generated by `room-service`; DM/botDM use `idgen.BuildDMRoomID(requester.ID, other.ID)` (sorted concat, order-independent).
- **`Name`**: channels — request `Name` if non-empty, else `composeAutoName(users, orgs, channels)` (literal request values joined by `", "`, truncated to 100 runes); DM/botDM — equal to `ID`. Display-name-per-subscriber for DM/botDM lives on `Subscription.SidebarName`, not `Room.Name`.
...
```go
func composeAutoName(users, orgs []string, channels []ChannelRef) string {
...
(Implementation detail; the test suite asserts the rune count.)
...
### AsyncJobResult (reuses or defines per PR #131)

```go
type AsyncJobResult struct {
    RequestID string `json:"requestId"`
    Operation string `json:"operation"`            // "room.create" | "room.member.add"
...
4. Build the Room doc:
   ...
   resolveRoomName:
     ...
     - channel with non-empty req.Name: return truncateRunes(req.Name, 100)
     - channel with empty req.Name: return composeAutoName(req.Users, req.Orgs,
       req.Channels)
...
room-service (site-A)
  ...
  - deterministicRoomID = idgen.BuildDMRoomID("u_alice", "u_bob") = "u_alice|u_bob"
  - dedup: FindDMSubscription(alice, "bob") → ErrSubscriptionNotFound
  - publish canonical event
  - reply {status:"accepted", roomId:"u_alice|u_bob", roomType:"dm"}

room-worker (site-A)
  step 4   Room{id:u_alice|u_bob, type:dm, name:u_alice|u_bob, siteID:site-A, createdBy:""}
  ...
  step 11  Outbox to site-B (single event):
             ...
             payload: {RoomID:"u_alice|u_bob", RoomType:"dm",
                       RoomName:"u_alice|u_bob", HomeSiteID:"site-A",
...
End state:
- Site-A: Room{id:u_alice|u_bob}, alice's sub, bob's sub (site-A copy).
...
| 11 | Channel auto-name truncation | `Users` very long, joined > 100 runes | publish; payload `Name` exactly 100 runes |
...
| 18 | Channel auto-name with channels | empty `Name`, request has `Channels=[ref]` | published `Name` includes `ref.RoomID` |
...
- Auto-naming applied when `req.Name == ""` → Room doc has the joined string truncated to 100 runes.
...
- New helpers (`composeName`, `composeAutoName`, `truncateRunes`, `stripAccount`, `subscriptionName`, `subscriptionSidebarName`, `subscriptionIsSubscribed`, `rolesForType`): ≥ 95% (pure functions).
