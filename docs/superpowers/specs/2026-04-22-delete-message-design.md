...
- **`rooms.LastMsgID` / `LastMsgAt` / `LastMentionAllAt` pointer staleness** — on soft-delete, the MongoDB `rooms` document is **not** updated. If the deleted message happened to be the room's most recent message, `LastMsgID` still points to the now-tombstoned row and `LastMsgAt` keeps its original timestamp. User-visible consequences:
  - The inbox in `chat-frontend/src/components/RoomList.jsx:19-21` sorts by `lastMsgAt` descending; the affected room stays at the same sort position it held before the delete, even though the preview will render as "[message deleted]".
  - If the deleted message had `mention_all`, `LastMentionAllAt` is not rolled back either — any UI badge driven by this field continues until a new message arrives.
  - This matches edit behavior (edit also does not touch `rooms`), keeps the delete handler free of extra MongoDB/Cassandra work, and avoids edge cases (empty-room rollback, race with a concurrent new message). If the UX proves problematic, a future PR can add roll-forward logic (query Cassandra for the latest `deleted = false` message in the room, update `rooms` in MongoDB, and extend `RoomMetadataUpdateEvent` to carry the refreshed fields for live inbox re-sort).
- **`threadRooms.LastMsgID` / `LastMsgAt` pointer staleness** — same non-update on soft-delete. Invisible to users today: `threadRooms` is not exposed by any API, not consumed by any frontend component, and not included in any published event — only `message-worker` reads and writes it for internal bookkeeping. No user-facing impact from staleness; no mitigation needed until these fields start being surfaced.
