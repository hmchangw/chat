...
## 14. Phase-1 research corrections (supersede earlier text where noted)

Deep research into the integration surface (search-sync-worker, search-service, pkg/atrest, history-service, tools/loadgen) corrected several assumptions baked into §4/§6/§7. These govern the Phase 1–3 plan:

1. **Approach A stores an *independent* atrest ciphertext, NOT "the exact same `enc_payload` as Cassandra" (supersedes §4/§6).** atrest is AES-256-GCM with a fresh random nonce per call, so re-encrypting the same content yields different bytes. search-sync-worker therefore produces its *own* atrest ciphertext+nonce for the same per-room DEK; search-service decrypts it normally. Byte-equality with Cassandra is neither achievable nor required. Consequence: two independent ciphertext copies exist (already noted as a risk), and **search-sync-worker gains new Vault + Mongo (DEK store) dependencies**, gated by the encryption flag.
2. **The `es:` tag reflection helper (`esPropertiesFromStruct`) cannot express `binary` / `index:false`.** The encrypted index template's `contentEnc`/`encNonce` mappings are authored explicitly (reflection is still used for the metadata fields).
3. **The blind key is loaded from an env var, not Vault.** atrest only does transit-wrap/unwrap; there is no Vault raw-secret read path in the codebase. The federation-wide blind key arrives as `BLINDIDX_KEY` (hex-encoded) + `BLINDIDX_KEY_VERSION`, **hex-decoded to raw bytes before `blindidx.New`** (carry-forward flag #3), and a shared `pkg/blindsearch` composes `msganalyzer.Analyze`+`blindidx` identically on both sides to guarantee index==query parity.
4. **Approach B requires a NEW history-service batch endpoint.** The decrypting `Repository.GetMessagesByIDs` exists but is internal; only a single-message, room-scoped `MsgGet` RPC is exposed. The plan adds a flat batch-by-IDs NATS subject + handler (decision: "Full: new batch RPC").
5. **Benchmark arm selection.** Arms C/A/B are one search-service serving three query shapes. `SearchMessagesRequest` gains an optional `Variant` field honored **only** when `SEARCH_BENCH_MODE_ENABLED=true`; in production the arm is fixed by config. This lets loadgen drive all three arms against a single instance over NATS for apples-to-apples latency.
6. **Indexing fan-out & failure semantics.** The encrypted doc is a *second* `BulkAction` emitted from the existing message collection's `BuildAction` (reusing the proven fan-out + ack-coupling), not a second consumer. Encryption failures (DEK/Vault) must NAK for redelivery, so the sync-worker handler is changed to distinguish `errcode.Permanent` (poison → ack-drop) from transient errors (→ NAK); existing validation failures become `Permanent`.

## 15. Phase-2 quality-harness findings (empirically confirmed against ES 8.17)

The offline quality harness (`tools/searchquality`) surfaced two real Go-`msganalyzer`-vs-ES-`custom_analyzer` divergences. Both are recorded here because they change how the §8a quality gate must be read.

1. **The production `custom_analyzer` does NOT bigram CJK — our Go analyzer DOES (and is therefore *better* at CJK).** Proven via `_analyze` + `explain:true`: ES's `cjk_bigram` token filter only fires on CJK-*typed* tokens (produced by `standard`/`icu` tokenizers), but the analyzer's `underscore_preserving` **pattern** tokenizer emits `word`-typed tokens, so `cjk_bigram` is a **no-op**. Example: `公园散步` → ES `[公园散步]` (one whole-run token) vs Go `[公园, 园散, 散步]` (bigrams). Pinned by `tools/searchquality` `TestCJKProbe`.
   - **Implication for the §8a gate:** the blind index (Go analyzer) does CJK substring matching the old plaintext index never could. So for CJK, **RBO/Jaccard "vs C" UNDER-credits the blind path** — RBO≈0 reflects C's weakness, not a blind-index defect (harness shows CJK recall@10 = 1.0 vs ground-truth). **For CJK, trust recall@10 against a ground-truth relevance set, not RBO-vs-C.** Because cutover *replaces* C with the blind index, this CJK improvement is what ships — no action needed beyond reading the gate correctly.
2. **HTML `&` (minor):** ES `html_strip` decodes `&amp;`→`&` and keeps `&` as a token; Go `msganalyzer` drops it (`world & peace` → ES keeps `&`, Go yields `world peace`). Benign — nobody searches a bare `&`; noted for completeness.

Net: the analyzer is *self-consistent* (index==query via `pkg/blindsearch`), which is the correctness requirement; bug-for-bug parity with the old Lucene analyzer was never the goal (§2/§7), and where it diverges here it diverges in the *helpful* direction (CJK).
