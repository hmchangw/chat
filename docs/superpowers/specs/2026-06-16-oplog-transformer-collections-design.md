...
- **insert / replace / update** → **insert-if-absent by account**. If the account already exists (another sync got there first), **leave it untouched** — so post-seed profile `update`s are intentionally **not** propagated (the company-wide user sync owns the record, §9). Field mapping (confirmed):
...
- **User post-seed `update`s** — not propagated (insert-if-absent; the company-wide user sync owns the record).
