...
| `tshow` | boolean | Present only when the request set `alsoSendToChannel: true` on a thread reply (absent when the flag was normalized away on a non-thread send). |
