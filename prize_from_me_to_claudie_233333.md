...
Note: org-membership is ONE row per (room, org), not per user. `member_removed` removes ALL users from that org's coverage in that room. If both `member_removed` and `member_added` fire in the same cycle for the same (room, org), the org row is briefly removed then re-added — safe because the worker is idempotent.
