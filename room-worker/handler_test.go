...
// Test 2: Invalid UUID → permanent error.
func TestProcessRoomVisibility_InvalidUUID(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	store := NewMockSubscriptionStore(ctrl)
...
// Test 3: Unmarshal failure → permanent + AsyncJobResult does NOT publish (empty requesterAccount).
func TestProcessRoomVisibility_UnmarshalFailure(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	store := NewMockSubscriptionStore(ctrl)
...
// Test 4: UpdateRoomVisibility returns ErrRoomNotFound → permanent + AsyncJobResult error.
func TestProcessRoomVisibility_RoomNotFound(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	store := NewMockSubscriptionStore(ctrl)

	requestID := "01970a4f-8c2d-7c9a-abcd-e0123456789f"
	store.EXPECT().UpdateRoomVisibility(gomock.Any(), "r1", true, false).Return(ErrRoomNotFound)
...
// Test 5: UpdateRoomVisibility returns ErrNotChannelRoom → permanent + AsyncJobResult error.
func TestProcessRoomVisibility_NotChannelRoom(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	store := NewMockSubscriptionStore(ctrl)

	requestID := "01970a4f-8c2d-7c9a-abcd-e0123456789f"
	store.EXPECT().UpdateRoomVisibility(gomock.Any(), "r1", true, false).Return(ErrNotChannelRoom)
...
// Test 6: Happy path no remote sites — UpdateRoomVisibility, ApplySubscriptionVisibility,
// ListByRoom, publishSubscriptionEvents, AsyncJobResult ok. No sys message published.
func TestProcessRoomVisibility_HappyPathNoRemoteSites(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	store := NewMockSubscriptionStore(ctrl)
...
// Test 6: Happy path with one remote site — outbox published with RoomVisibilityOutboxPayload.
...
// Test 7: Error-then-ok retry sequence.
func TestProcessRoomVisibility_ErrorThenOkRetrySequence(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	store := NewMockSubscriptionStore(ctrl)
	requestID := "01970a4f-8c2d-7c9a-abcd-e0123456789f"

	store.EXPECT().UpdateRoomVisibility(gomock.Any(), "r1", true, false).Return(errors.New("mongo timeout"))
	store.EXPECT().UpdateRoomVisibility(gomock.Any(), "r1", true, false).Return(nil)
