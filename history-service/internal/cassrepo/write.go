...
const (
	...
	// attachments, card, card_action, quoted_parent_message). buildEditPayload
	...
	// rows. sys_msg_data is NOT encrypted, so it is preserved (not nulled).
	...
	editPinnedMsgEncrypted = `UPDATE pinned_messages_by_room SET enc_payload = ?, enc_meta = ?, msg = null, attachments = null, card = null, card_action = null, quoted_parent_message = null, edited_at = ?, updated_at = ? WHERE room_id = ? AND created_at = ? AND message_id = ?`
