...
| `--abort-window-max-samples=N`    | 10000 (auto-sized) | Cap on the abort/progress latency ring buffer (S3); drop-oldest when full. `0` disables the cap (legacy unbounded behavior). Bounds the per-tick percentile sort: at the default 10k, each sort is ~10k log 10k ≈ 130k comparisons (~150µs). **Bug 6 fix — auto-sizing:** when this flag is NOT explicitly set, the resolved cap is `2 × max(--rate, --ramp-to) × max(--abort-p99-sustain, --abort-error-sustain)` (in seconds), so the watcher cannot be deafened by the registered default. An explicit value (including `0`) is honored verbatim. The auto-sized resolution is logged at run start (`abort-window-max-samples auto-sized`). **Coverage trade-off (still applies to explicit values):** when `peak_rps × max(--abort-p99-sustain, --abort-error-sustain) > cap`, retention is compressed below the sustain interval, so the abort watcher's `covered` check returns false and it cannot fire even under a real sustained breach. The watcher emits `slog.Warn` `"abort watcher deafened by sample cap"` so a silent no-fire is detectable; raise the cap or set to `0` to keep the watcher functional. Rule of thumb: `cap ≥ peak_rps × max_sustain`. |
