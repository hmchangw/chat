...
| `nats: message does not have a reply` in room-service | Loadgen action handler used `Publish` instead of `Request` for a subject room-service responds on. | Use the latest loadgen — `readReceipt` was fixed in commit `0bde680` to use `Request`. |
