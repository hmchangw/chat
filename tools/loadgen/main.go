...
func main() {
	...
	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: loadgen <seed|run|teardown|members-sustained|members-capacity|history-sustained|search-sustained|max-rps|daily> [flags]")
		os.Exit(2)
	}
...
func dispatch(ctx context.Context, cfg *config) int {
	...
	case "search-sustained":
		return runSearchSustained(ctx, cfg, os.Args[2:])
	case "max-rps":
