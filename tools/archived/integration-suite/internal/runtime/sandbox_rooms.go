...
// `subscriptions`, and `room_members`. Field shapes mirror the
// production Go structs (pkg/model.Room / .Subscription / .RoomMember)
// per docs/spec-room-subscription-seed.md §3 — written via bson.M so
// the harness stays black-box and pkg/model is untouched.
//
// Preconditions (the caller — Sandbox.Setup — guarantees these):
//   - sb.Deps.Mongo is non-nil.
//   - sb.Scenario.Seed has already been passed through
//     scenario.ValidateSeedBlock, so rule violations (duplicate ids,
//     DM arity, undeclared room refs, bogus enums) are impossible.
//   - sb.Users is populated (every alias referenced by a membership
//     resolves to a SeedUser); siteID is non-empty.
//   - sb.StartTime captures T_open (sandbox session boundary).
//
// All inserts within a collection are issued via a single InsertMany
// to keep the number of round-trips bounded. Iteration order is
// deterministic (sorted by alias / by member uid) so test assertions
// on `uids` / `accounts` arrays are stable across runs.
func insertSeededRooms(ctx context.Context, sb *Sandbox) error {
	if len(sb.Scenario.Seed.Rooms) == 0 {
		return nil
	}

	siteID := sb.Deps.SiteID
	if siteID == "" {
		siteID = defaultSiteID
	}
	tOpen := sb.StartTime.UTC()

	roomsByID := indexRoomsByID(sb.Scenario.Seed.Rooms)
	membersByRoom := membershipsByRoom(sb.Scenario.Seed.Memberships)

	roomDocs, err := buildRoomDocs(sb.Scenario.Seed.Rooms, membersByRoom, sb.Users, siteID, tOpen)
	if err != nil {
		return fmt.Errorf("build room docs: %w", err)
	}
	if _, err := sb.Deps.Mongo.Collection("rooms").InsertMany(ctx, roomDocs); err != nil {
		return fmt.Errorf("insert rooms: %w", err)
	}

	subDocs, memDocs := buildSubscriptionAndMemberDocs(sb.Scenario.Seed.Memberships, roomsByID, sb.Users, siteID, tOpen)
	if len(subDocs) > 0 {
		if _, err := sb.Deps.Mongo.Collection("subscriptions").InsertMany(ctx, subDocs); err != nil {
			return fmt.Errorf("insert subscriptions: %w", err)
		}
	}
	if len(memDocs) > 0 {
		if _, err := sb.Deps.Mongo.Collection("room_members").InsertMany(ctx, memDocs); err != nil {
			return fmt.Errorf("insert room_members: %w", err)
		}
	}
	return nil
}
