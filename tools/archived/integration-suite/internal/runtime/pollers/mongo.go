...
// NewMongoFindPoller builds the singleton primitive. Register it
// under "mongo_find" in the runtime registry.
func NewMongoFindPoller(db *mongo.Database, startTime time.Time) *MongoFindPoller {
	return &MongoFindPoller{db: db, startTime: startTime}
}
...
// sandbox isolation.
func (p *MongoFindPoller) PollFn(args map[string]any, _ string) func() []readers.Event {
	...
	coll := p.db.Collection(collection)
