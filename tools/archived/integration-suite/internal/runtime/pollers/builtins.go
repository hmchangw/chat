...
type BuiltinDeps struct {
	// MongoDB is the chat database handle. Required for `mongo_find`.
...
func RegisterBuiltinPollers(reg *Registry, deps *BuiltinDeps) (cleanup func(), err error) {
	...
	// mongo_find — stateless. Registered even when deps.MongoDB is
	// nil; PollFn nil-checks and warns.
	reg.Register("mongo_find", NewMongoFindPoller(deps.MongoDB, deps.StartTime))
