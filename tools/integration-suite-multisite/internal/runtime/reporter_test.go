...
// --- DRAFT↔finding linkage (#6) ---
// The pass/fail semantics rule says draft + fail = open finding. The
// reporter makes the linkage VISIBLE: parse the findings doc, match
// scenarios named in "Demonstrated by" lines, render an Open findings
// section. Unlinked DRAFT fails surface as Untriaged.

func TestParseFindings_ExtractsScenarioReferences(t *testing.T) {
	doc := "" +
		"## F-005 — malformed requestId\n" +
		"Some body text describing the bug.\n" +
		"\n" +
		"Demonstrated by\n" +
		"`scenarios/drafts/gatekeeper-validation/gatekeeper-malformed-requestid-silent-drop.yaml`\n" +
		"(green).\n" +
		"\n" +
		"## F-006 — quote drop\n" +
		"Body.\n" +
		"`scenarios/drafts/quote/quote-nonexistent-parent-drops-message.yaml`\n"
	idx := ParseFindings(doc)
	assert.Equal(t, "F-005", idx["gatekeeper-malformed-requestid-silent-drop"])
	assert.Equal(t, "F-006", idx["quote-nonexistent-parent-drops-message"])
}

func TestParseFindings_MultipleScenariosUnderOneFinding(t *testing.T) {
	doc := "" +
		"## F-006 — quote drop\n" +
		"`scenarios/drafts/quote/quote-nonexistent-parent-drops-message.yaml`\n" +
		"`scenarios/drafts/quote/quote-fresh-parent.yaml`\n"
	idx := ParseFindings(doc)
	assert.Equal(t, "F-006", idx["quote-nonexistent-parent-drops-message"])
	assert.Equal(t, "F-006", idx["quote-fresh-parent"])
}

func TestParseFindings_NoFindingsHeading_EmptyIndex(t *testing.T) {
	idx := ParseFindings("just some text with no F-NNN headings\n`scenarios/drafts/x.yaml`")
	assert.Empty(t, idx)
}

func TestRender_OpenFindingsSection_LinkedDraftFail(t *testing.T) {
	r := &RunReport{
		StartISO: "2026-06-14T00:00:00Z",
		Cases: []ScenarioReport{
			{ScenarioName: "gatekeeper-malformed-requestid-silent-drop",
				SourcePath: "scenarios/drafts/gatekeeper-validation/gatekeeper-malformed-requestid-silent-drop.yaml",
				Status: "draft", Kind: "negative",
				Verdict: Verdict{Outcome: "fail"}},
		},
	}
	idx := FindingsIndex{"gatekeeper-malformed-requestid-silent-drop": "F-005"}
	out := RenderWithFindings(r, false, idx)
	assert.Contains(t, out, "Open findings")
	assert.Contains(t, out, "F-005")
	assert.Contains(t, out, "gatekeeper-malformed-requestid-silent-drop")
}

func TestRender_UntriagedSection_UnlinkedDraftFail(t *testing.T) {
	r := &RunReport{
		StartISO: "2026-06-14T00:00:00Z",
		Cases: []ScenarioReport{
			{ScenarioName: "mystery-red",
				SourcePath: "scenarios/drafts/foo/mystery-red.yaml",
				Status: "draft", Kind: "negative",
				Verdict: Verdict{Outcome: "fail"}},
		},
	}
	idx := FindingsIndex{}
	out := RenderWithFindings(r, false, idx)
	assert.Contains(t, out, "Untriaged DRAFT fails")
	assert.Contains(t, out, "mystery-red")
	assert.Contains(t, out, "file a F-NNN")
}

func TestRender_PassingScenariosNotInFindingsSection(t *testing.T) {
	r := &RunReport{
		StartISO: "2026-06-14T00:00:00Z",
		Cases: []ScenarioReport{
			{ScenarioName: "green-scn",
				SourcePath: "scenarios/drafts/x.yaml",
				Status: "draft", Kind: "positive",
				Verdict: Verdict{Outcome: "pass"}},
		},
	}
	idx := FindingsIndex{"green-scn": "F-099"}
	out := RenderWithFindings(r, false, idx)
	assert.NotContains(t, out, "## Open findings")
	assert.NotContains(t, out, "## Untriaged DRAFT fails")
}

func TestRender_ApprovedFailDoesNotBucketAsOpenFinding(t *testing.T) {
	r := &RunReport{
		StartISO: "2026-06-14T00:00:00Z",
		Cases: []ScenarioReport{
			{ScenarioName: "approved-broke",
				SourcePath: "scenarios/drafts/foo/approved-broke.yaml",
				Status: "approved", Kind: "negative",
				Verdict: Verdict{Outcome: "fail"}},
		},
	}
	idx := FindingsIndex{"approved-broke": "F-005"}
	out := RenderWithFindings(r, false, idx)
	assert.NotContains(t, out, "## Open findings")
}

func TestRender_NoFindingsIndex_NoSectionAdded(t *testing.T) {
	r := &RunReport{
		StartISO: "2026-06-14T00:00:00Z",
		Cases: []ScenarioReport{
			{ScenarioName: "x", SourcePath: "scenarios/drafts/x.yaml",
				Status: "draft", Kind: "negative",
				Verdict: Verdict{Outcome: "fail"}},
		},
	}
