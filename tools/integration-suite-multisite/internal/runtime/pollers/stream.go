...
// events. site, args, and traceparent are accepted to satisfy the
// Poller interface but are ignored — the StreamPoller is a
// fixed-binding helper; per-args dispatch is handled by the universal
// primitives that wrap it (JetStreamConsumePoller, LogsTailPoller,
// ReplyPoller).
func (p *StreamPoller) PollFn(_ string, _ map[string]any, _ string) func() []readers.Event {
