...
func runScenario(ctx context.Context, s *scenario.Scenario, deps *runnerDeps) error {
	...
	// references (spec §5).
	if err := fireTasks(ctx, s.Input, sb.Deps.Dispatcher, &subCtx, sb.Users, sb.Deps.Services); err != nil {
		dur := time.Since(start)
		recordScenario(deps.Perf, deps.Report, s,
...
//
//nolint:gocritic // hugeParam: scenario.Expected copied per call by design.
