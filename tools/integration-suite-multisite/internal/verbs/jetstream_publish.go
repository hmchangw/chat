...
func NewJetStreamPublish(siteURLs map[string]string) *JetStreamPublish {
	return &JetStreamPublish{SiteURLs: siteURLs, Timeout: 5 * time.Second}
}
...
func (n *JetStreamPublish) Execute(ctx context.Context, in *Input) Outcome {
	...
	if !ok || url == "" {
		return Outcome{
			Err:       fmt.Errorf("jetstream_publish: no NATS URL for site %q", in.Site),
			RequestID: requestID,
		}
	}
