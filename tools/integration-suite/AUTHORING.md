...
(note: the framework now lives in `internal/harness/`, a split the
spec predates — see README "Layout").

You only ever touch the top-level surface: `features/*.feature` and
`*_steps_test.go`. Step files dot-import `internal/harness`, so write
steps unqualified (`suiteWorld`, `natsRequest`, `ClassNone`) exactly as
the skeleton below shows. Never edit `internal/harness/`.
...
RULE 2: If the design is silent, tag the scenario @blindspot:<slug>
        and add an entry to docs/integration-suite-v1/blindspots.md.
...
- `@covers:<slug>` — claims a documented case in
  `docs/integration-suite-v1/coverage.md`. Slug must have a matching
  `## <slug>` there. A scenario may carry several `@covers:` tags.
...
- [ ] If `@blindspot:<slug>`, matching entry exists in `docs/integration-suite-v1/blindspots.md`
- [ ] If the behavior is a catalogued case, `@covers:<slug>` is set and the
      slug exists in `docs/integration-suite-v1/coverage.md`
