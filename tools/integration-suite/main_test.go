...
func writeHumanSummary(startedAt time.Time, duration time.Duration) error {
	...
	// Append the report-only coverage block (skipped if no register).
	if md, err := os.ReadFile(filepath.Join(targetDir, "coverage.md")); err == nil {
		if rep, cerr := ScoreCoverage(doc, ParseCoverageRegister(string(md))); cerr == nil {
...
func InitializeScenario(ctx *godog.ScenarioContext) {
	...
	registerRoleSteps(ctx)
	registerHistorySteps(ctx)
	registerMessagingSteps(ctx)
	registerMessagePersistenceSteps(ctx)
