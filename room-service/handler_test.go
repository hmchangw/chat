...
func TestHandler_RemoveMember_SelfLeave_Success(t *testing.T) {
	...
	var publishedData []byte
	handler := NewHandler(store, nil, nil, nil, nil, "site-a", 1000, 500, 5*time.Second, func(ctx context.Context, subj string, data []byte) error {
		publishedSubj = subj
		publishedData = data
		return nil
	}, nil)
...
func TestHandler_RemoveMember_OrgOnly_Rejected(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			handler := NewHandler(store, nil, nil, nil, nil, "site-a", 1000, 500, 5*time.Second, nil, nil)
...
func TestHandler_RemoveMember_SelfLeave_NoOrgs_Allowed(t *testing.T) {
	...
	var publishedData []byte
	handler := NewHandler(store, nil, nil, nil, nil, "site-a", 1000, 500, 5*time.Second, func(ctx context.Context, _ string, data []byte) error {
		publishedData = data
		return nil
	}, nil)
	reqSubj := subject.MemberRemove("alice", "r1", "site-a")
...
func TestHandler_RemoveMember_PublishError(t *testing.T) {
	...
	store.EXPECT().CountMembersAndOwners(gomock.Any(), "r1").
		Return(&RoomCounts{MemberCount: 3, OwnerCount: 2}, nil)
	handler := NewHandler(store, nil, nil, nil, nil, "site-a", 1000, 500, 5*time.Second, func(_ context.Context, _ string, _ []byte) error {
...
func TestHandler_AddMembers_RestrictedOwnerAllowed(t *testing.T) {
	...
	h := NewHandler(store, nil, nil, nil, nil, "site-a", 100, 500, 5*time.Second, publish, nil)
...
func TestHandler_natsGetRoomKey(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			h := NewHandler(store, ks, nil, nil, nil, siteID, 1000, 500, 5*time.Second, nil, nil)
