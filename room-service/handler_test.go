...
func TestHandler_AddMembers_RestrictedOwnerAllowed(t *testing.T) {
	...
	h := NewHandler(store, nil, nil, nil, "site-a", 100, 500, 5*time.Second, publish)
...
func TestHandler_handleListRoomKeys(t *testing.T) {
	privKey1 := []byte{1, 2, 3, 4}
	privKey2 := []byte{5, 6, 7, 8}
	...
	tests := []struct {
		...
		setupKeys  func(*MockRoomKeyStore)
		wantErr    string
		assertResp func(t *testing.T, resp model.RoomsKeysResponse)
	}{
		{
			name:    "happy path — 2 subscribed rooms, both keyed",
			account: "alice",
			setupStore: func(s *MockRoomStore) {
				s.EXPECT().
					ListSubscriptionsByAccount(gomock.Any(), "alice").
			...
			setupKeys: func(k *MockRoomKeyStore) {
				k.EXPECT().
					...
					Return(map[string]*roomkeystore.VersionedKeyPair{
						"r1": {Version: 7, KeyPair: roomkeystore.RoomKeyPair{PrivateKey: privKey1}},
						"r2": {Version: 3, KeyPair: roomkeystore.RoomKeyPair{PrivateKey: privKey2}},
					}, nil)
			},
			assertResp: func(t *testing.T, resp model.RoomsKeysResponse) {
				t.Helper()
				require.Len(t, resp.Keys, 2)
				byRoom := map[string]model.RoomsKeysEntry{}
				for _, k := range resp.Keys {
					byRoom[k.RoomID] = k
				}
				require.Contains(t, byRoom, "r1")
				assert.Equal(t, 7, byRoom["r1"].Version)
				assert.Equal(t, privKey1, byRoom["r1"].PrivateKey)
				require.Contains(t, byRoom, "r2")
				assert.Equal(t, 3, byRoom["r2"].Version)
				assert.Equal(t, privKey2, byRoom["r2"].PrivateKey)
			},
		},
		{
			name:    "room in subscriptions has no key in Valkey — skipped in response",
			account: "alice",
			setupStore: func(s *MockRoomStore) {
				s.EXPECT().
					ListSubscriptionsByAccount(gomock.Any(), "alice").
			...
			setupKeys: func(k *MockRoomKeyStore) {
				k.EXPECT().
					GetMany(gomock.Any(), []string{"r1", "r2"}).
					Return(map[string]*roomkeystore.VersionedKeyPair{
						"r1": {Version: 1, KeyPair: roomkeystore.RoomKeyPair{PrivateKey: privKey1}},
						// r2 absent → nil value
						"r2": nil,
					}, nil)
			},
			assertResp: func(t *testing.T, resp model.RoomsKeysResponse) {
				...
				assert.Equal(t, "r1", resp.Keys[0].RoomID)
			},
		},
		{
			name:    "no subscriptions — returns empty Keys slice",
			account: "alice",
			setupStore: func(s *MockRoomStore) {
				s.EXPECT().
					ListSubscriptionsByAccount(gomock.Any(), "alice").
		...
		{
			name:    "store error — propagated as error",
			account: "alice",
			setupStore: func(s *MockRoomStore) {
				s.EXPECT().
					ListSubscriptionsByAccount(gomock.Any(), "alice").
			...
			setupKeys: func(k *MockRoomKeyStore) {},
			wantErr:   "list subscriptions for alice",
		},
		{
			name:    "keyStore error — propagated as error",
			account: "alice",
			setupStore: func(s *MockRoomStore) {
				s.EXPECT().
					ListSubscriptionsByAccount(gomock.Any(), "alice").
			...
			setupKeys: func(k *MockRoomKeyStore) {
				k.EXPECT().
					...
					Return(nil, fmt.Errorf("valkey down"))
			},
			wantErr: "get room keys for alice",
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			respBytes, err := h.handleListRoomKeys(context.Background(), tc.account)
