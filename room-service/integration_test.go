...
func TestMongoStore_ListSubscriptionsByAccount_ReturnsAllSubscriptionsRegardlessOfIsSubscribed_Integration(t *testing.T) {
	ctx := context.Background()
	db := setupMongo(t)
	store := NewMongoStore(db)
	require.NoError(t, store.EnsureIndexes(ctx))

	// Seed four subscriptions for "alice" covering all room types and both
	// IsSubscribed values — the bug under test filtered on isSubscribed:true,
	// which would have excluded channels, DMs, and the bot side of botDMs.
	aliceSubs := []model.Subscription{
		{
			ID:       "s-channel",
			User:     model.SubscriptionUser{ID: "u-alice", Account: "alice"},
			RoomID:   "r-channel",
			RoomType: model.RoomTypeChannel,
			// IsSubscribed defaults to false — channels never set it true
		},
		{
			ID:       "s-dm",
			User:     model.SubscriptionUser{ID: "u-alice", Account: "alice"},
			...
			// IsSubscribed defaults to false — DMs never set it true
		},
		{
			ID:           "s-botdm-human",
			User:         model.SubscriptionUser{ID: "u-alice", Account: "alice"},
			...
			IsSubscribed: true, // human side of botDM is the only case where IsSubscribed=true
		},
		{
			ID:       "s-botdm-bot",
			User:     model.SubscriptionUser{ID: "u-alice", Account: "alice"},
			RoomID:   "r-botdm-bot",
			RoomType: model.RoomTypeBotDM,
			// IsSubscribed defaults to false — bot side is always false
		},
	}
	for i := range aliceSubs {
		require.NoError(t, store.CreateSubscription(ctx, &aliceSubs[i]),
			"seeding alice subscription %q", aliceSubs[i].ID)
	}

	// Seed one subscription for "bob" to verify the account filter is applied.
	bobSub := model.Subscription{
		ID:       "s-bob",
		User:     model.SubscriptionUser{ID: "u-bob", Account: "bob"},
		RoomID:   "r-channel",
		RoomType: model.RoomTypeChannel,
	}
	require.NoError(t, store.CreateSubscription(ctx, &bobSub))

	result, err := store.ListSubscriptionsByAccount(ctx, "alice")
	require.NoError(t, err)

	// ALL four alice subscriptions must be returned regardless of IsSubscribed.
	assert.Len(t, result, 4, "expected all 4 alice subscriptions; isSubscribed filter would exclude some")

	// Verify the four expected room IDs are present (order-independent).
	...
	for _, sub := range result {
		assert.True(t, wantRoomIDs[sub.RoomID],
			"unexpected roomID %q in result", sub.RoomID)
		assert.Equal(t, "alice", sub.User.Account,
			"all returned subs must belong to alice")
	}

	// Bob's subscription must not appear.
	for _, sub := range result {
		assert.NotEqual(t, "bob", sub.User.Account,
			"bob's subscription leaked into alice's results")
	}
}
