...
func (h *Handler) handleCreateRoom(ctx context.Context, data []byte) ([]byte, error) {
	...
	case model.RoomTypeDM:
		...
		if req.Members[0] == req.CreatedBy {
			return nil, fmt.Errorf("DM requires a different recipient than the creator")
		}
	...
	var recipSub model.Subscription
	if req.Type == model.RoomTypeDM {
	...
	if h.publishEvent != nil {
		...
		// Mirror the notification to the DM recipient so their UI updates too.
		if req.Type == model.RoomTypeDM {
			recipEvt := model.SubscriptionUpdateEvent{
				UserID:       recipSub.User.ID,
				Subscription: recipSub,
				Action:       "added",
				Timestamp:    now.UnixMilli(),
			}
			if recipEvtData, err := json.Marshal(recipEvt); err != nil {
				slog.Warn("marshal recipient subscription update event failed", "error", err, "roomID", room.ID)
			} else if err := h.publishEvent(ctx, subject.SubscriptionUpdate(req.Members[0]), recipEvtData); err != nil {
				slog.Warn("publish recipient subscription update failed", "error", err, "roomID", room.ID)
			}
		}
	}
