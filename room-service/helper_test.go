...
func TestSanitizeError(t *testing.T) {
	tests := []struct {
		...
		{"user-facing: only owners", fmt.Errorf("only owners can remove"), "only owners can remove"},
		{"user-facing: capacity", fmt.Errorf("room is at maximum capacity (1000)"), "room is at maximum capacity (1000)"},
		{"user-facing: invalid role", fmt.Errorf("invalid role: \"admin\""), "invalid role: \"admin\""},
		{"internal error", fmt.Errorf("mongo connection failed"), "internal error"},
		{"wrapped internal", fmt.Errorf("create subscription: %w", fmt.Errorf("timeout")), "internal error"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
